...
/*   Updated: 2024/10/04 02:41:54 by joandre-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	mutex_init(t_table *tab)
{
	...
	if (pthread_mutex_init(&tab->log, NULL)
		...
		return (errmsg("pthread_mutex_init", NULL), false);
	i = 0;
	while (i < tab->p_count)
	{
...
static bool	simulation_time(t_table *tab)
{
	...
	if (gettimeofday(&tab->startime, NULL))
		return (errmsg("gettimeofday", NULL), free(tab->p), false);
	while (i < tab->p_count)
		tab->p[i++].last_meal = tab->startime;
	set_bool(&tab->main, &tab->start, true, tab);
	return (true);
}
...
bool	sim_init(t_table *tab)
{
	...
	if (tab->p == NULL)
		return (errmsg("malloc", NULL), false);
	if (!mutex_init(tab))
		return (free(tab->p), false);
	if (pthread_create(&tab->host, NULL, monitor, tab))
		return (errmsg("pthread_create", NULL), free(tab->p), false);
	i = 0;
	while (i < tab->p_count)
	{
		tab->p[i].id = i + 1;
		...
		if (pthread_create(&tab->p[i].philo, NULL, diner, &tab->p[i]))
			return (errmsg("pthread_create", tab), free(tab->p), false);
		++i;
	}
...
int	sim_exit(t_table *tab)
{
	...
	if (pthread_mutex_destroy(&tab->log) || pthread_mutex_destroy(&tab->main)
		...
		errmsg("pthread_mutex_destroy", tab);
	i = 0;
	while (i < tab->p_count)
	{
