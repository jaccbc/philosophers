...
/*   Updated: 2024/09/25 17:56:17 by joandre-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	valid_range(char *nbr)
{
	...
	if (!nbr)
		return (errmsg("null"), false);
	n = 0;
	while (*nbr)
	{
		n = n * 10 + (*nbr++ - '0');
		if (n > INT_MAX)
			return (errmsg("Number out of range"), false);
	}
	return (true);
}
...
static bool	table_check(int ac, char **av, t_table *tab)
{
	...
	if (tab->p_count == 0)
		return (errmsg("Can't be 0 philosophers"), (false));
	...
	if (ac == 6)
	{
		...
		if (tab->must_eat == 0)
			return (errmsg("Can't eat 0 times"), (false));
	}
	return (true);
}
...
static bool	argv_check(int ac, char **av, t_table *tab)
{
	...
	while (t < ac)
	{
		i = 0;
		if (!av[t][i])
			return (errmsg("Null string"), false);
		while (av[t][i] >= '0' && av[t][i] <= '9')
			++i;
		if (av[t][i])
			return (errmsg("Numbers only"), false);
		if (!valid_range(av[t++]))
			return (false);
	}
...
int	main(int argc, char **argv)
{
	...
	if (argc < 5 || argc > 6)
		return (errmsg("Wrong number of arguments"), (-1));
